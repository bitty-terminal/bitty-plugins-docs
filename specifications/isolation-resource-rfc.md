...
| RC-5  | Event queue depth (three-level budgets) | per-subscription / per-plugin / global | PerSubscription 64 events per `(plugin, event-type)` queue; PerPlugin 1024 events / 256 KiB aggregate; Global 8192 events / 2 MiB aggregate (accepted defaults per [Plugin Platform RFC](plugin-platform-rfc.md), OQ-011/OQ-012/OQ-013, 2026-08-27; OQ-014 retains enforcement tuning) | PerSubscription enforced strictly at `EventQueue::push`; PerPlugin enforced at `EventPipeline::publish` via DropPolicy (Wave-C P1 decision DropOldest as v1 default, DropNewest alternative — accepted) — P0 review for enforcement tuning required; Global is accepted admission-control open item (not yet hard-gated, exposed via `total_queued_events`/`total_queued_bytes` and `bitty plugin doctor`); overflow drop policy is the single authoritative statement in [Plugin Platform RFC](plugin-platform-rfc.md#delivery-ordering-batching-and-coalescing) (accepted); drops counted and attributed |
...
- Queue budgets use three levels (PerSubscription 64, PerPlugin 1024 events /
  ...
  [Plugin Platform RFC](plugin-platform-rfc.md) (OQ-011/OQ-012/OQ-013,
  2026-08-27); PerSubscription is strict FIFO, PerPlugin uses the authoritative
  DropPolicy (DropOldest default, DropNewest alternative) at the plugin
  boundary, Global is an accepted admission-control open item.
