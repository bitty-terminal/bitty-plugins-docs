...
> observation/event systems; **PerSubscription 64, PerPlugin 1024 events/256 KiB,
> Global 8192 events/2 MiB** are the aligned three-level queue budgets
> (candidate, OQ-014). This note is candidate-winner evidence only and does not
> advance the RFC beyond Proposed.
...
Out of scope: capability identifiers and manifest schema (OQ-012), Plugin API
surface (OQ-011), event interception phases (OQ-013), image protocol limits
...
Status: **proposed initial values**. None is measured; each follows the
[Performance Budget RFC](performance-budget-rfc.md) convention that numbers are
target contracts for future implementation and measurement harnesses. Tests
...
| RC-5  | Event queue depth (three-level budgets) | per-subscription / per-plugin / global | PerSubscription 64 events per `(plugin, event-type)` queue; PerPlugin 1024 events / 256 KiB aggregate; Global 8192 events / 2 MiB aggregate (all candidate, OQ-014) | PerSubscription enforced strictly at `EventQueue::push`; PerPlugin enforced at `EventPipeline::publish` via DropPolicy (Wave-C P1 selects DropOldest as v1 default, DropNewest alternative) — candidate, P0 review required; Global is candidate admission-control open item (not yet hard-gated, exposed via `total_queued_events`/`total_queued_bytes` and `bitty plugin doctor`); overflow drop policy is the single authoritative statement in [Plugin Platform RFC](plugin-platform-rfc.md#delivery-ordering-batching-and-coalescing); drops counted and attributed |
...
- Queue budgets use three levels (PerSubscription 64, PerPlugin 1024 events /
  ...
  `bitty-plugin-host/src/event.rs`; PerSubscription is strict FIFO, PerPlugin
  uses the authoritative DropPolicy (DropOldest default, DropNewest alternative)
  at the plugin boundary, Global is a candidate admission-control open item.
...
- Given a plugin spawning maximal tasks and timers while flooding its event
  ...
  (Wave-C P1 selects DropOldest as v1 default for UI observation/event systems,
  DropNewest alternative) with drops counted and attributed, no drop is silent,
...
dedicated mutation-attempt cases remain owned by the Plugin API and capability
RFCs (OQ-011/OQ-012).
