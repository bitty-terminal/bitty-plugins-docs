...
description: Proposes isolation boundaries resource ceilings and failure semantics for PTY processes plugins and IPC/MCP clients with an adversarial test specification
...
> with `BoundedText` strict, per Plugin Platform RFC; OQ-014 retains
> enforcement tuning). This note was candidate-winner evidence before
> acceptance and is now the accepted contract for the queue-budget defaults.
>
...
> admission control, not hard-gated), `BoundedText` 8 KiB strict, and
> `drain_batch` strict (`<= max_bytes`); instrumentation
> `event.rs`/`host.rs` (`budget_snapshot`, `invariant_queue_bounds`,
> `invariant_global_bounds`, `publish_count`). Lifecycle remains
> `Draft -> experimental review evidence -> Accepted -> normative`; queue-budget
> defaults are accepted but OQ-014 enforcement tuning remains `Proposed`
> pending P0 review, while RC-1/RC-2 and global enforcement stay candidate
> `Open`.
...
Status: **proposed initial values with measurement evidence for queue budgets
(2026-08-27)**. Per-subscription 64, per-plugin 1024 events / 256 KiB
(enforced at `EventPipeline::publish` via `DropOldest`), `BoundedText` 8 KiB,
and `drain_batch` strict are now measured via
`bitty/crates/bitty-plugin-host/tests/measurement.rs` (17 headless tests;
instrumentation `event.rs`/`host.rs`: `budget_snapshot`,
`invariant_queue_bounds`/`invariant_global_bounds`, `publish_count`) as
experimental review evidence per bitty CTX-0037 PR #68 (docs CTX-0049); global
8192 events / 2 MiB is tracked via `total_queued_events`/`total_queued_bytes`
for future host admission control but not hard-gated. RC-1 instruction/CPU,
RC-2 memory, and global admission-control enforcement remain candidate `Open`
pending Lua VM / P0 review. All values follow the
...
value requires an RFC revision, never silent drift. Lifecycle is
...
| ----- | --------------------------------------- | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RC-1  | Callback CPU/instruction budget         | each plugin callback                   | 10^7 VM instructions or 50 ms wall clock, whichever first (candidate Open, pending Lua VM — no enforcement claimed)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Open — no VM wired, no enforcement claimed; constants `RC1_*` exposed only for harness parameterization; pending `piccolo` watch-list / P0 review, warning candidate at 8 ms per plugin-system measurement note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| RC-2  | Memory per plugin VM                    | each plugin                            | 32 MiB accounted allocations (candidate Open, pending Lua VM — no enforcement claimed)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Open — no allocator accounting yet, no VM wired; pending measurement and `piccolo` watch-list / P0 review; floor 8 MiB; policy maximum 256 MiB                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| RC-5  | Event queue depth (three-level budgets) | per-subscription / per-plugin / global | PerSubscription 64 events per `(plugin, event-type)` queue; PerPlugin 1024 events / 256 KiB aggregate; Global 8192 events / 2 MiB aggregate (accepted defaults per [Plugin Platform RFC](plugin-platform-rfc.md), OQ-011/OQ-012/OQ-013, 2026-08-27; OQ-014 retains enforcement tuning; measured 2026-08-27 via `bitty/crates/bitty-plugin-host/tests/measurement.rs` 17 headless tests and `event.rs`/`host.rs` instrumentation `budget_snapshot`/`invariant_*` — per-subscription strict at `EventQueue::push`, per-plugin at `EventPipeline::publish` via `DropOldest`, global tracked via `total_queued_*`) | PerSubscription enforced strictly at `EventQueue::push` (measured); PerPlugin enforced at `EventPipeline::publish` via DropPolicy (Wave-C P1 decision DropOldest as v1 default, DropNewest alternative — accepted) — measured via harness, experimental review evidence per bitty CTX-0037 PR #68 / docs CTX-0049, P0 review for enforcement tuning required before normative, lifecycle `Draft -> experimental review evidence -> Accepted -> normative`; Global is accepted admission-control open item (not yet hard-gated, exposed via `total_queued_events`/`total_queued_bytes` and `bitty plugin doctor`, tracking measured not gated, candidate Open); overflow drop policy is the single authoritative statement in [Plugin Platform RFC](plugin-platform-rfc.md#delivery-ordering-batching-and-coalescing) (accepted); drops counted and attributed |
...
- Queue budgets use three levels (PerSubscription 64, PerPlugin 1024 events /
  ...
  alternative) at the plugin boundary (`EventPipeline::publish`), Global is an
  accepted admission-control open item (tracked via `total_queued_*`, not
  hard-gated). Measured 2026-08-27 via
  `bitty/crates/bitty-plugin-host/tests/measurement.rs` (17 headless tests,
  instrumentation `budget_snapshot`/`invariant_*`/`publish_count`, bounded
  `BoundedText` 8 KiB and `drain_batch` strict) as experimental review evidence
  per bitty CTX-0037 PR #68 / docs CTX-0049; queue budgets remain candidate
  `Proposed` pending P0 review, lifecycle
  `Draft -> experimental review evidence -> Accepted -> normative`; RC-1/RC-2
  and global enforcement stay candidate `Open` pending Lua VM / P0 review.
...
- Migrated or deferred (remain Open as follow-up work, not as OQ-014 closure
  ...
  **instruction/CPU budgets (RC-1 10^7 VM instructions / 50 ms wall clock,
  warning candidate 8 ms)** — Open candidate, no Lua VM wired, no enforcement
  claimed, constants `RC1_*` exposed only for harness parameterization, pending
  `piccolo` watch-list / P0 review (follow-up CTX-0038); **per-plugin bytes and
  aggregate enforcement (RC-5 PerPlugin 256 KiB / 1024 events, enforced at
  `EventPipeline::publish` via DropPolicy)** — now measured via
  `bitty/crates/bitty-plugin-host/tests/measurement.rs` headless harness and
  `event.rs`/`host.rs` instrumentation (`budget_snapshot`,
  `invariant_queue_bounds`/`invariant_global_bounds`, `publish_count`, 17
  tests, bitty CTX-0037 PR #68 / docs CTX-0049, 2026-08-27) as experimental
  review evidence but still candidate `Proposed` pending P0 review for
  enforcement tuning before normative; **global limits (Global 8192 events /
  2 MiB aggregate, accepted admission-control open item, not yet hard-gated,
  exposed via `total_queued_events`/`total_queued_bytes` and `bitty plugin
doctor`)** — tracked not gated, tracking correctness measured in harness,
  enforcement via host admission control remains Open candidate; **memory
  ceilings (RC-2 32 MiB per plugin VM)** — Open candidate, no allocator
  accounting yet, pending Lua VM / P0 review; concrete VM technology and
  instruction-counting mechanism (`mlua` wrapper today versus `piccolo`
  watch-list) closing the hook left by
...
`publish_count`) and docs CTX-0049 (2026-08-27) provide experimental review
evidence proving per-subscription 64 strict at `EventQueue::push`, per-plugin
1024 events / 256 KiB at `EventPipeline::publish` via `DropOldest`, global
8192 events / 2 MiB tracking via `total_queued_*` (not hard-gated),
`BoundedText` 8 KiB, and `drain_batch` strict; queue budgets remain candidate
`Proposed` pending P0 review for enforcement tuning, while RC-1
instruction/CPU budgets, RC-2 memory ceilings, and global admission-control
enforcement remain Open candidates pending Lua VM (`piccolo` watch-list) and
reference-hardware measurement; lifecycle is
`Draft -> experimental review evidence -> Accepted -> normative`.
